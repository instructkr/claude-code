//! Web search and fetch tools for multi-agent workflows.
//!
//! This module provides HTTP-based tools for fetching web content and
//! performing web searches. These tools can be used by agents to gather
...
pub fn execute_web_fetch(input: &WebFetchInput) -> Result<WebFetchOutput, String> {
...
pub fn execute_web_search(input: &WebSearchInput) -> Result<WebSearchOutput, String> {
    let started = Instant::now();
    let client = build_http_client()?;
