...
353. **Top-level `plugins uninstall <missing-plugin> --output-format json` hangs with zero stdout/stderr instead of returning a typed plugin-not-found response** — dogfooded 2026-04-29 for the 23:30 nudge on current `origin/main` / rebuilt `./rust/target/debug/claw` with embedded `git_sha` `6f92e54d`. After rebuilding and verifying the binary provenance, repeated bounded runs of `timeout 8 ./rust/target/debug/claw plugins uninstall does-not-exist --output-format json` exited `124` with `stdout=0` and `stderr=0`. In the same rebuilt binary, `plugins list --output-format json` returned promptly with the known plugin inventory payload, proving the plugin command surface is reachable and narrowing the hang to missing-target lifecycle uninstall. This is distinct from #350's missing-target `plugins enable` timeout and from #351/#352 where disable/update fail fast with JSON on stderr: uninstall is another destructive supported lifecycle verb and currently joins the silent-hang class. **Required fix shape:** (a) validate the target plugin against discovered/configured inventory before invoking uninstall-side effects or cleanup hooks; (b) return bounded JSON with `kind:"plugin"` or `domain:"plugin"`, `action:"uninstall"`, `status:"not_found"` or `code:"plugin_not_found"`, `plugin`, and optional `available_plugins[]`; (c) share a missing-target guard across enable/uninstall and reconcile lifecycle error stream/status semantics across disable/update as well; (d) add regression coverage proving `plugins uninstall does-not-exist --output-format json` returns a typed JSON outcome within a deterministic budget and does not mutate plugin state. **Why this matters:** uninstall is the most destructive plugin lifecycle action. A stale or typoed plugin name must fail safely before cleanup hooks or loader work; a zero-byte hang forces claws to kill the process and cannot distinguish absent target, registry stall, cleanup deadlock, or dispatch bug. Source: gaebal-gajae dogfood follow-up for the 23:30 nudge on rebuilt `./rust/target/debug/claw` `6f92e54d`.
