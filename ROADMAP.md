...
385. **Top-level `init --help --output-format json` exits successfully but emits plain text help instead of JSON** — dogfooded 2026-04-30 for the 05:00 nudge on current `origin/main` / local workspace while the checked-out `./rust/target/debug/claw` binary still reported embedded `git_sha` `d95b230c`. A fresh probe on the startup/setup surface showed a prompt-but-wrong contract: `./rust/target/debug/claw init --help --output-format json` exited `0` with `stdout=324` and `stderr=0`, but stdout began with plain text `Init`, `Usage`, `Purpose`, `Output`, `Formats`, and `Related`; attempting to parse stdout as JSON failed at byte 0. In the same workspace, `version --output-format json` returned promptly with version/build metadata, proving the binary itself and JSON output path are reachable. This is distinct from zero-byte help hangs (#358/#380/#381/#382/#383) and from the export plaintext-help case (#384): the affected surface is startup/setup discovery for `init`, where claws need machine-readable mutation/setup semantics before creating `.claw/`, `.claw.json`, `.gitignore`, or `CLAUDE.md`. **Required fix shape:** (a) make `init --help --output-format json` emit valid stdout JSON with `kind:"help"` or `kind:"init"`, `action:"help"`, usage, options, defaults, output formats, examples, planned created/updated artifact categories, and related slash/direct commands; (b) preserve text help for default/text mode only; (c) add a `format:"json"` or equivalent field so callers can assert the contract without parsing prose; (d) add regression coverage proving init help with JSON format parses as JSON and does not silently fall back to plain text. **Why this matters:** `init` is a mutation-heavy onboarding command. If JSON help accepts the flag but returns prose, automation cannot safely preflight what setup will create or update before touching the workspace, and startup/setup remains transcript scraping instead of a bounded machine-readable contract. Source: gaebal-gajae dogfood follow-up for the 05:00 nudge on rebuilt/local `./rust/target/debug/claw` `d95b230c`.
