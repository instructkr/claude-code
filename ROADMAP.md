...
414. **`status --output-format json` returns `git_state:"clean"` in a non-git directory — indistinguishable from a clean git repo, while `git_branch:null` and `project_root:null`** — dogfooded 2026-04-30 by Jobdori on `e939777f`. Running `claw status --output-format json` from `/tmp/probe-nogit` (confirmed `fatal: not a git repository` from `git status`) returns `workspace.git_state:"clean"`, `workspace.git_branch:null`, `workspace.project_root:null`, `workspace.changed_files:0`, `workspace.staged_files:0`, `workspace.unstaged_files:0`, `workspace.untracked_files:0`. The same `git_state:"clean"` value appears in a real git repo with no uncommitted changes. Automation that reads `git_state:"clean"` cannot tell whether it is inside a clean git repo or simply outside any git repo entirely — both states are reported with the same value and zero counts. **Required fix shape:** (a) add a `git_available:bool` or `is_git_repo:bool` field to `workspace` so the non-git case is unambiguously machine-readable without parsing `git_branch:null`; (b) use a distinct `git_state` value for the non-git case such as `"no_repository"` or `"not_applicable"` instead of `"clean"`, which implies a git repo exists and is clean; (c) alternatively, return a `git` object with `{available:false, reason:"not_a_repo"}` when outside a repo, and `{available:true, state:"clean", branch:"main", ...}` when inside one; (d) add regression coverage proving `status --output-format json` in a non-git directory returns a different and unambiguously non-git `git_state` value than the value returned in a clean git repo. **Why this matters:** session initialization and workspace safety checks depend on knowing whether a project is a git repo before committing, staging, or running git-dependent workflows. If `git_state:"clean"` means both "clean repo" and "no repo", automation must parse `git_branch:null` as a heuristic or probe `git status` independently — defeating the purpose of structured status output. Source: Jobdori live dogfood, `e939777f`, 2026-04-30.
