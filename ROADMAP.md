...
365. **`/status --output-format json` `allowed_tools.entries: null` is ambiguous: callers cannot distinguish "all tools allowed" from "tool list not loaded / unavailable"** — dogfooded 2026-04-30 by Jobdori on `0f9e8915`. Running `/status --output-format json` returns `{"allowed_tools":{"entries":null,"restricted":false,"source":"default"}}`. The `entries` field is `null` rather than an empty array `[]` or an explicit "all" sentinel. Three possible interpretations are all consistent with `null`: (a) all tools are allowed (unrestricted); (b) the tool list was not loaded or is undefined; (c) the runtime could not enumerate available tools. The companion field `restricted: false` provides some indication but does not resolve whether `null` means "list is irrelevant" or "list is unknown". An orchestrator checking whether a specific tool is available cannot distinguish a null allowlist (all-pass) from a failed enumeration (unknown). **Required fix shape:** (a) when `restricted: false`, either set `entries: null` with a documented semantic of "all tools permitted" in the schema, or return an explicit `entries: []` for "no restrictions" with a separate `allow_all: true` flag; (b) add `has_tool_allowlist: bool` field to disambiguate; (c) if the tool list could not be loaded, emit `entries_load_error: "reason"` and set `entries: null` with `restricted: null` (unknown); (d) add regression coverage confirming the three states (all-allowed, restricted with list, and load-error) are distinguishable from `entries` and companion fields. Source: Jobdori live dogfood, mengmotaHost, `0f9e8915`, 2026-04-30.
