...
339. **`/session delete` is not resume-safe while `/session list` is, making session GC impossible from `--resume` mode automation** — dogfooded 2026-04-30 by Jobdori on current main (`24ccb59`). Running `claw --output-format json --resume latest /session list` succeeds and returns the full session list. Running `claw --output-format json --resume latest /session delete <id>` returns `{"command":"/session delete ...","error":"unsupported resumed slash command","type":"error"}`. An orchestration lane that uses `--resume` + `--output-format json` to introspect and manage sessions can list sessions but cannot delete any of them — making session GC impossible without spawning an interactive REPL session just to issue the delete command. This is a concrete operational gap: all 10 listed sessions show `workspace_dirty: true, abandoned: true`; an automated cleanup lane cannot reclaim disk space via the same `--resume` path it uses to discover sessions. Additionally, the error response again uses `"type"` instead of `"kind"` (#336 root cause). **Required fix shape:** (a) mark `/session delete` as resume-safe in the slash command registry so it can be executed from `--resume latest /session delete <id>`; (b) return a `kind=session_deleted` JSON object on success with `{"kind": "session_deleted", "session_id": "<id>", "path": "<deleted_path>"}` so automation can confirm deletion; (c) require `--force` only when deleting the currently-active session or a session with `workspace_dirty: true` and no explicit acknowledgment; (d) add regression coverage proving `/session delete <old-id>` works in `--resume` mode and returns structured JSON. **Why this matters:** the entire session lifecycle — list, switch, delete — should be automatable from a single `--resume` invocation; forcing an interactive REPL session just to delete a stale session defeats the purpose of the machine-readable JSON surface. Source: Jobdori live dogfood on mengmotaHost, claw-code `24ccb59`, 2026-04-30.
